...
system("export APP_NAME=#{app_name} && export DATA_LOCATION=./tmp/data && export DB_LOCATION=./tmp/db && export DEPLOYMENT_STAGE=development && export POSTGRES_PASSWORD=#{app_name} && docker-compose run  --rm web rails new . --api --force --no-deps --database=postgresql")
...
# add simple_token_authentication to Gemfile
system("echo \"gem 'simple_token_authentication'\" >> Gemfile")
